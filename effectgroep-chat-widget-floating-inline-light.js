(function(window, document) {
    ...
    const EffectgroepChatFloatingLoader = {
        config: {
        ...
        init: function(userConfig = {}) {
            ...
            if (!this.config.inboxToken) {
                console.error('EffectgroepChatFloating: inboxToken is required');
                return;
            }
        ...
        createIframe: function() {
            ...
            floatingContainer.style.cssText = 'position:fixed;bottom:2rem;left:50%;transform:translateX(-50%);width:100%;max-width:46rem;padding:0 1rem;z-index:998;pointer-events:none;';

            const iframe = document.createElement('iframe');
            iframe.id = 'effectgroep-chat-floating-iframe';
            iframe.style.cssText = 'width:100%;height:140px;border:none;overflow:hidden;background:transparent;pointer-events:auto;';
            ...
            const iframeContent = this.generateFloatingIframeContent();
...
<head>
    ...
    <style>
        * {
        ...
        :root {
            --eg-pink: ${colors.sendButton};
            --eg-neutral-darker: #ffffff;
            --eg-neutral-dark: #ffffff;
            --eg-white: #2C285E;
            --eg-text-muted: #CACACA;
            --eg-font-family: 'proxima-nova', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
        }
        ...
        .ai-chat::after {
            ...
            height: calc(100% + 2px);
            border-radius: 80px;
            background: linear-gradient(45deg, ${glowGradient});
            background-size: 400%;
            z-index: 20;
            animation: glowbox 20s linear infinite;
        }
        ...
        .input-container {
            background-color: var(--eg-neutral-darker);
            width: 100%;
            height: 72px;
            padding: 1rem;
            position: relative;
        ...
        .chat-input {
            flex: 1;
            background-color: var(--eg-neutral-dark);
            border: none;
            outline: none;
            font-size: 1rem;
            font-weight: 500;
            padding: 0 75px 0 2rem;
            height: 100%;
            color: var(--eg-white);
            font-family: var(--eg-font-family);
            z-index: 100;
        ...
        .send-btn:hover {
            ...
            box-shadow: 0 0 20px ${hexToRgba(colors.sendButtonHoverGlow, colors.sendButtonHoverGlowOpacity)};
        }
            ...
            <div class="ai-chat">
                ...
                <button type="button" class="send-btn" id="send-btn" aria-label="Verstuur">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 17 19">
                        <path fill="${colors.arrow}" d="M1.57.43c.26-.03.53.03.76.16l13.62,7.78c.2.11.37.28.48.48.12.2.18.42.18.65,0,.23-.06.46-.18.66-.12.2-.28.36-.49.48l-13.62,7.79c-.2.11-.42.17-.64.17-.21,0-.41-.05-.59-.15-.18-.1-.34-.23-.46-.4-.12-.17-.2-.36-.22-.57-.03-.2,0-.41.06-.61l2.02-6c.71-.52,1.55-.88,2.47-1.01l2.35-.34-2.35-.34c-.92-.13-1.76-.49-2.47-1.01L.47,2.15c-.09-.25-.1-.52-.03-.77.07-.25.21-.48.42-.65.2-.17.45-.27.71-.3Z"/>
                    </svg>
                </button>
                </div>
            </div>
    ...
    <style>
        * {
        ...
        :root {
            --eg-pink: ${colors.sendButton};
            --eg-off-black: #101010;
            --eg-neutral-darker: #222;
            --eg-neutral-dark: #444;
            --eg-neutral-darkest: #111;
            --eg-white: #ffffff;
            --eg-text-muted: rgba(255, 255, 255, 0.6);
            --eg-font-family: 'proxima-nova', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
        }
        ...
        .modal-overlay.active .modal-background {
        ...
        .message-wrapper .message-text p:last-child {
            margin-bottom: 0;
        }

        .message-wrapper .message-text ol,
        .message-wrapper .message-text ul {
            margin: 1rem 0;
            padding-left: 1.5rem;
        }

        .message-wrapper .message-text li {
            margin-bottom: 0.5rem;
        }

        .message-wrapper .message-text strong {
            font-weight: 700;
        }

        .message-wrapper .message-text em {
            font-style: italic;
        }

        .message-wrapper .message-text code {
        ...
        .message-wrapper .message-text a {
            color: ${colors.agentMessageLink};
            text-decoration: underline;
        }

        .message-wrapper.user-message {
            background-color: ${colors.userMessage};
            border-radius: 1rem;
            max-width: 75%;
            padding: 1rem;
            margin-left: auto;
        ...
        .message-wrapper.user-message .message-text a {
            color: var(--eg-white);
        }
...
<body>
    ...
    window.EffectgroepChatFloating = EffectgroepChatFloatingLoader;

    EffectgroepChatFloating.init();
})(window, document);
